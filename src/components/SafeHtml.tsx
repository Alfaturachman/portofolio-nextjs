...
const ALLOWED_TAGS = new Set([
    'p', 'a', 'strong', 'b', 'em', 'i', 'code', 'pre', 'span',
    ...
    while (lastIndex < html.length) {
        ...
        if (isClosing) {
            ...
            tokens.push({
                ...
                name: name.toLowerCase(),
                attrs,
                selfClosing: !!selfClose || VOID_ELEMENTS.has(name.toLowerCase()),
                closing: false,
            } as TagToken)
        }
...
function buildTree(tokens: Token[], index = 0): { nodes: React.ReactNode[]; nextIndex: number } {
...
function createElement(token: TagToken, children: React.ReactNode[]): React.ReactNode {
    ...
    return React.createElement(token.name, Object.keys(props).length > 0 ? props : null, ...children)
}
